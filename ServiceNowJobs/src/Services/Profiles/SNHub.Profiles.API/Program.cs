...
using SNHub.Profiles.API.Middleware;
using SNHub.Profiles.Infrastructure.Extensions;
...
try
{
    ...
    builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(SNHub.Profiles.Application.Commands.UpsertCandidateProfile.UpsertCandidateProfileCommand).Assembly));
    builder.Services.AddValidatorsFromAssembly(typeof(SNHub.Profiles.Application.Commands.UpsertCandidateProfile.UpsertCandidateProfileCommandValidator).Assembly);
    ...
    if (app.Configuration.GetValue<bool>("RunMigrationsOnStartup")) { using var scope = app.Services.CreateScope(); await scope.ServiceProvider.GetRequiredService<ProfilesDbContext>().Database.MigrateAsync(); }
    app.UseMiddleware<ExceptionMiddleware>(); app.UseSerilogRequestLogging();
    if (app.Environment.IsDevelopment()) { app.MapOpenApi(); app.MapScalarApiReference(o => { o.Title = "SNHub Profiles API"; o.Theme = ScalarTheme.DeepSpace; }); }
...
catch (Exception ex) { Log.Fatal(ex, "Profiles Service failed."); throw; }
