...
public static class InfrastructureExtensions
{
    ...
    {
        var conn = config.GetConnectionString("JobsDb") ?? throw new InvalidOperationException("JobsDb required.");
        services.AddDbContext<JobsDbContext>(o =>
            o.UseNpgsql(conn, n => { n.EnableRetryOnFailure(3); n.MigrationsHistoryTable("__ef_migrations", "jobs"); }));
        services.AddScoped<IUnitOfWork>(sp => sp.GetRequiredService<JobsDbContext>());
        services.AddScoped<IJobRepository, JobRepository>();
        services.AddHealthChecks().AddNpgSql(conn, name: "jobs-postgres", tags: ["ready"]);
        return services;
    }
}
