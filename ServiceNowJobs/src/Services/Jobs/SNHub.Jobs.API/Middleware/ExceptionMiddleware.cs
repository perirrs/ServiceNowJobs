...
public sealed class ExceptionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        catch (JobNotFoundException ex)  { await WriteError(ctx, 404, ex.Message); }
        catch (JobAccessDeniedException) { await WriteError(ctx, 403, "You do not have permission to modify this job."); }
        catch (JobNotActiveException ex) { await WriteError(ctx, 409, ex.Message); }
