...
using SNHub.Users.API.Middleware;
using SNHub.Users.Infrastructure.Extensions;
...
try
{
    ...
    builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(SNHub.Users.Application.Commands.UpdateProfile.UpdateProfileCommand).Assembly));
    builder.Services.AddValidatorsFromAssembly(typeof(SNHub.Users.Application.Commands.UpdateProfile.UpdateProfileCommandValidator).Assembly);
    ...
    if (app.Configuration.GetValue<bool>("RunMigrationsOnStartup"))
    {
        ...
        Log.Information("Users DB migrated.");
    }
    ...
    {
        app.MapOpenApi();
        app.MapScalarApiReference(o => { o.Title = "SNHub Users API"; o.Theme = ScalarTheme.DeepSpace; });
    }
    ...
    Log.Information("SNHub Users Service ready on {Env}", app.Environment.EnvironmentName);
    await app.RunAsync();
}
catch (Exception ex) { Log.Fatal(ex, "Users Service failed to start."); throw; }
