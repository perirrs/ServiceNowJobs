...
public sealed class NotificationRepository : INotificationRepository
{
    ...
    public Task<Notification?> GetByIdAsync(Guid id, CancellationToken ct = default) => _db.Notifications.FindAsync([id], ct).AsTask();
    public async Task AddAsync(Notification n, CancellationToken ct = default) => await _db.Notifications.AddAsync(n, ct);
    public async Task<(IEnumerable<Notification> Items, int Total, int UnreadCount)> GetByUserAsync(Guid userId, bool? unreadOnly, int page, int pageSize, CancellationToken ct = default)
    {
    ...
    public async Task MarkAllAsReadAsync(Guid userId, CancellationToken ct = default)
    {
        await _db.Notifications.Where(n => n.UserId == userId && !n.IsRead)
            .ExecuteUpdateAsync(s => s.SetProperty(n => n.IsRead, true).SetProperty(n => n.ReadAt, DateTimeOffset.UtcNow), ct);
    }
}
